...
def runDemo():
    ...
    parser.add_argument(
        "--orphanedSetting", help="This setting does not belong to a group :("
    )
    ...
    numericSettings.add_argument(
        "--int", type=int, default=100, help="Decimals are not allowed"
    )
    numericSettings.add_argument("--float", type=float, help="Decimals are allowed")
    numericSettings.add_argument(
        "--pickInt", type=int, choices=[1, 2, 3], help="Choose one of these"
    )
    numericSettings.add_argument(
        "--pickFloat",
        type=float,
    ...
    exoticSettings = parser.add_argument_group(
        "Exotic types", description="Fancy data types"
    )
    ...
    exoticSettings.add_argument(
        "--path", type=pathlib.Path, default=pathlib.Path.home() / "file.txt"
    )
